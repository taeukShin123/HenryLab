...
public class GridShufflerInScene : MonoBehaviour {

	public GridShuffler sMaze;
	...
	GridShufflerInScene.enPivotpoint PivotSelection;
	
	
	
	public void GetMazeInSceneProperties(GridShuffler MazeProperties) {
		...
		GoSimpleMazePiece = MazeProperties.ModuleList[0].GoModule;
		GoMaze = MazeProperties.GoMaze;
		...
		PivotSelection = (GridShufflerInScene.enPivotpoint)MazeProperties.Pivot;
	...
	public GridShuffler UpdateInSceneMazeProperties() {
		UpdatePivotPosition();
		this.name = MazeName + ": " + (IndexNumber-1)  + " (" + MazeWidth + "x" + MazeHeigth + " ) " + PivotSelection;
		return sMaze;
	}
	/*
